...
import { useValidateForm } from "~/routes/admin/guard/registro";

export default component$(() => {
  const errorFlag = useSignal(false);
  const errorMessage = useSignal("");

  const action = useValidateForm();
  ...
  return (
    <>
      ...
      >
        ...
        {errorFlag.value && (
          <p class="text-xl font-semibold text-red-500">{errorMessage.value}</p>
        )}
      ...
      <Modal showModal={modalVisible.value} closeFn={closeModal}>
        ...
        <div q:slot="content">Guardia agregado</div>
      </Modal>
    </>
  );
});
